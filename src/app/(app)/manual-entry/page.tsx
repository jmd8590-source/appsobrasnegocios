...
const CATEGORY_BADGES: Record<MaterialCategory, string> = {
...
export default function ManualEntryPage() {
  ...
  const [selectedMaterialName, setSelectedMaterialName] = useState<string>(CATALOG_MATERIALS[0].name);
  
  // Custom material modal state
  const [isCustomMaterial, setIsCustomMaterial] = useState(false);
  const [customName, setCustomName] = useState('');
  const [customCategory, setCustomCategory] = useState<MaterialCategory>('metal');
  const [customPrice, setCustomPrice] = useState<number>(1.0);

  // Form fields
  const [quantity, setQuantity] = useState<string>('10');
  const [unit, setUnit] = useState<'kg' | 'ud'>('kg');
  const [customRate, setCustomRate] = useState<string>('');
  const [conditionNotes, setConditionNotes] = useState<string>('');
  const [status, setStatus] = useState<ScrapStatus>('available');

  const [isSaving, setIsSaving] = useState(false);
  const [showSuccessModal, setShowSuccessModal] = useState(false);
  const [lastSavedScrap, setLastSavedScrap] = useState<Partial<Scrap> | null>(null);
  ...
  const currentMaterial = useMemo<CatalogMaterialItem>(() => {
  ...
  const activePrice = useMemo<number>(() => {
          ...
          <Card className="border-slate-800 bg-slate-900/90 shadow-lg">
            <CardContent className="p-5 space-y-4">
              ...
              <div className="flex gap-1.5 overflow-x-auto pb-1 text-xs">
                ...
                {(['metal', 'wood', 'plastic', 'construction'] as MaterialCategory[]).map((cat) => (
                  <button
                    key={cat}
                    type="button"
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) => setCustomCategory(e.target.value as MaterialCategory)}
                ...
                <div className="flex flex-wrap gap-1.5">
                  {[1, 5, 10, 25, 50, 100, 250, 500].map((val) => (
                    <button
                      ...
                      className="px-2.5 py-1 text-xs rounded-lg bg-slate-800 border border-slate-700 text-slate-300 hover:bg-slate-700 hover:text-amber-400 transition-colors"
                    >
                      +{val} {unit}
                    </button>
                  ))}
                  <button
                    ...
                    className="px-2.5 py-1 text-xs rounded-lg bg-slate-950 border border-slate-800 text-slate-500 hover:text-red-400 transition-colors"
                  >
                    Borrar
                  </button>
                </div>
            ...
            <CardContent className="p-5 space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <select
                    value={status}
                    onChange={(e) => setStatus(e.target.value as ScrapStatus)}
